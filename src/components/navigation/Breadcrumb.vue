...
const fetchCategoriaInfo = async (categoriaId) => {
  ...
  try {
    const response = await services.getCategoria(route.params.slug, categoriaId);
    if (response?.data) {
...
const fetchSubcategoriaInfo = async (subcategoriaId) => {
  ...
  try {
    const response = await services.getSubcategoria(route.params.slug, subcategoriaId);
    if (response?.data) {
...
watch(
  ...
  async () => {
    try {
      ...
      } else if (route.name === 'store-subcategoria' && route.params.subcategoriaId) {
