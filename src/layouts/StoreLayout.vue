  ...
  try {
    ...
    if (store.value.tema) {
      document.documentElement.style.setProperty(
        "--color-primary",
        store.value.tema.color_primario,
      );
      document.documentElement.style.setProperty(
        "--color-secondary",
        store.value.tema.color_secundario,
      );
    }
  } catch (err) {
...
<template>
  <div class="min-h-screen">
    ...
    <template v-else-if="store">
      <NavBar :store="store" @search="handleSearch" />
      <main>
        <router-view :store="store" :search-query="searchQuery"></router-view>
      </main>
      <ShoppingCart />
    </template>
  </div>
