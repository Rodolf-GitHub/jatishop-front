...
const fetchProducto = async () => {
  ...
  try {
    loading.value = true;
    error.value = null;
    const response = await services.getProducto(
      route.params.slug,
      route.params.productoId,
    );
    producto.value = response.data;

    const imagenUrl = new URL(producto.value.imagen, window.location.origin)
      .href;
...
const getPrecioFinal = () => {
  ...
  return (
    ...
    (1 - producto.value?.descuento / 100)
  ).toFixed(2);
};
...
<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div
      v-else-if="error"
      class="text-center text-red-500 py-8 bg-red-50 rounded-lg"
    >
      {{ error }}
    </div>
        ...
        <div class="flex flex-col p-8">
          ...
          <div class="mt-8 space-y-6">
            ...
            <div class="flex items-center space-x-4">
              ...
              <div
                class="flex items-center justify-center bg-gray-50 rounded-lg"
              >
                <button
          ...
          <button
            ...
            class="mt-8 w-full bg-gradient-to-r from-jati to-shop text-white py-4 px-6 rounded-lg text-lg font-semibold hover:opacity-90 transition-opacity disabled:opacity-50 disabled:cursor-not-allowed shadow-lg"
          >
          ...
          <div class="bg-gray-50 p-6 rounded-xl">
            ...
            <div v-if="producto.subcategoria_info" class="space-y-2 mt-2">
              <p class="flex items-center gap-2">
                ...
                <span class="text-jati">{{
                  producto.subcategoria_info.nombre
                }}</span>
              </p>
            </div>
          </div>
