<template>
  <div>
    <div class="mb-8">
      <h1 class="text-2xl font-bold text-white mb-2">Información del Negocio</h1>
      <p class="text-gray-400">Gestiona la información principal de tu tienda</p>
    </div>

    <div class="bg-gray-800 rounded-xl p-6 border border-gray-700">
      <form @submit.prevent="saveBusinessInfo" class="space-y-6">
        <!-- Logo -->
        <div>
          ...
          <div class="flex items-center space-x-6">
            <div class="w-24 h-24 rounded-lg bg-gray-700 flex items-center justify-center">
              <img v-if="businessInfo.logo" :src="businessInfo.logo" alt="Logo" class="w-20 h-20 object-contain">
              <i v-else class="fas fa-store text-4xl text-gray-500"></i>
            </div>
            <button type="button" class="px-4 py-2 bg-gray-700 text-gray-300 rounded-lg hover:bg-gray-600">
              Cambiar Logo
            </button>
          </div>
        </div>
...
const saveBusinessInfo = () => {
  ...
  console.log('Guardando información:', businessInfo.value);
};
</script> 